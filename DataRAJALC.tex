...
\begin{figure}
\begin{lstlisting}[caption={Algorithm for constructing decision space and constraints.}]
create_variables(view, model) 
	// variables for the format choices
	for kernel in kernels:
		for format in formats:
			model.add_format_variable(view,format,kernel)

	// variables for the conversion choices. 
	// there is one conversion time before each kernel and one after the last kernel
	for conversion_time in conversion_times
		for in_format in formats:
			for out_format in formats:
				model.add_conversion_variable(view,in_format,out_format,conversion_time)

add_constraints(view, model):

	//Format Uniqueness
	for kernel in kernels:
		sum = new Sum()
		for format in formats:
			sum += model.get_format_variable(view, format, kernel)
		model.add_equality_constraint(1,sum)
	...
	for conversion_time in conversion_times:
		...
		for in_format in formats:
			for out_format in formats:
				sum += model.get_conversion_variable(view, in_format, out_format, conversion_time)
		model.add_equality_constraint(1, sum)
	
	// Format-Conversion Matching, input
	for format_variable in model.format_variables:
		conversion_time = format_variable.next_conversion
		in_format = format_variable.format
		sum = new Sum()
		for out_format in formats:
			sum += model.get_conversion_variable(view, in_format, out_format, conversion_time)
		model.add_equality_constraint(format_variable, sum)

	// Format-Conversion Matching, output
	for format_variable in model.format_variables:
		conversion_time = format_variable.prior_conversion
		out_format = format_variable.format
		sum = new Sum()
		for in_format in formats:
			sum += model.get_conversion_variable(view, in_format, out_format, conversion_time)
		model.add_equality_constraint(format_variable, sum)	
	...
	// User Prescription
	for (format,kernel) in user_decisions:
		format_variable = model.get_format_variable(view, format, kernel)
		model.add_equality_constaint(format_variable, 1)
\end{lstlisting}
\end{figure}

\begin{figure}
\begin{lstlisting}[caption={Algorithm for constructing objective function.}]
add_objective_function(view, model):
	obj_func = new ObjectiveFunction()
	
	//cost estimates for conversions
	for conversion_variable in model.conversion_variables:
	...
	model.set_objective_function(obj_func)
\end{lstlisting}
\end{figure}
