...
Listing~\ref{SpaceAndConstraints} shows the pseudocode algorithm for creating the decision space and the constraints that ensure the final choice is both valid and respects the format selections made by the user.
Listing~\ref{AlgObjFunc} shows the psuedocode algorithm for constructing the objective function based on performance estimates. 
Both algorithms generate models for each view in the computation independently. 


\begin{figure}
\begin{lstlisting}[caption={Algorithm for constructing decision space and constraints.},label={SpaceAndConstraints}]
create_variables(view):
	# variables for the fmt choices
	for kernel in kernels:
		for fmt in formats:
			add_format_variable(view,fmt,kernel)

	# variables for the conversion choices. 
	# there is one conv time before each 
	# kernel and one after the last kernel
	for conv_time in conversion_times:
		for in_fmt in formats:
			for out_fmt in formats:
				add_conv_variable(view,in_fmt,out_fmt,conv_time)

add_constraints(view):

	# Format Uniqueness
	for kernel in kernels:
		sum = new Sum()
		for fmt in formats:
			sum += get_format_variable(view, fmt, kernel)
		add_equality_constraint(1,sum)
	...
	for conv_time in conversion_times:
		...
		for in_fmt in formats:
			for out_fmt in formats:
				sum += get_conv_variable(view, in_fmt, out_fmt, conv_time)
		add_equality_constraint(1, sum)
	
	# Format-Conversion Matching, input
	for format_variable in format_variables:
		conv_time = format_variable.next_conversion
		in_fmt = format_variable.format
		sum = new Sum()
		for out_fmt in fmts:
			sum += get_conv_variable(view, in_fmt, out_fmt, conv_time)
		add_equality_constraint(format_variable, sum)

	# Format-Conversion Matching, output
	for format_variable in format_variables:
		conv_time = format_variable.prior_conv
		out_fmt = format_variable.format
		sum = new Sum()
		for in_fmt in fmts:
			sum += get_conv_variable(view, in_fmt, out_fmt, conv_time)
		add_equality_constraint(format_variable, sum)	
	...
	# User Prescription
	for (fmt,kernel) in user_decisions:
		format_variable = get_format_variable(view, fmt, kernel)
		add_equality_constaint(format_variable, 1)
\end{lstlisting}
\Description[Algorithm for creating the model's decision variables and constraints.]{Algorithms for two functions, create variables and add constraints.}
\end{figure}
...
\end{lstlisting}
\Description{Algorithm for the function to create the model's objective function. First it adds cost estimates for conveersions, then adds cost estimates for the accesses in the computation.}
\end{figure}
