...
As the graph shows, there is significant performance improvement ($1.73\times$ speedup) simply by changing the data layout, and the cost of performing the layout change is small ($1.72\times$ speedup when including conversion time). 

\begin{figure}
	\includegraphics[width=\columnwidth]{ScoreValidity/IntroExampleGraph.pdf}
...
(3) typically do not optimize across multiple loops well, motivating a variety of approaches~\cite{ragan-kelley2013halide,mullapudi2015polymage,sujeeth2014delite,luporini2018design}.

Libraries like Kokkos and RAJA and programming languages like Chapel~\cite{diaconescu2007approach} give programmers some control over the data layout of multi-dimensional arrays, but this support is limited to the point of initialization.
...
While the user can specify as many format requirements as they desire, any left unspecified are completed using our extension's performance model.
Guided by run-time microbenchmarking, our performance model selects remaining format changes by solving an Integer Linear Programming (ILP) problem that considers the cost of converting formats against the potential improvement of using a more optimal format.
Our inclusion of a performance model means a developer can get performance improvement without having to determine the sequence of format changes themselves.

This paper includes the following contributions:
\begin{itemize}
\item An API in RAJA for specifying data layouts and data layout transformations.
\item A performance model to select a data layout and data layout transformation strategy for a loop chain.
\item An optimization that increases the reuse of microbenchmarking information.
\item Performance results and source lines of code (SLOC) for five variants of four benchmark kernels.
\end{itemize} 
...
\begin{figure}
	\includegraphics[width=\columnwidth]{RAJAPerf/PolybenchPerfPlot.pdf}
	\caption{Speedup of the Polybench variants relative to the RAJA\_OpenMP variant. (Higher is better)}
...
For the 2MM and 3MM benchmarks, the results show a meaningful speedup for the three variants that make layout changes (7.7\% to 8.3\% for 2MM and 7.0\% to 7.2\% for 3MM).
For MVT and GEMVER, because no layout changes are made, we do not see meaningful performance improvement (<1\% change).
...
This is because the \verb.Hand_Layout. variant is identical to the \verb.RAJALC. implementation, while the \verb.Format_Decision. and \verb.Model_Choice. variants instantiate the \FormatDecisions object.

\begin{figure}
	\includegraphics[width=\columnwidth]{RAJAPerf/PolybenchSLOC.pdf}
	\caption{Source lines of code modified (added, changed, or removed) relative to the RAJA\_OpenMP variant. (Lower is better)}
	\label{PolybenchSLOC}
	\Description[Polybench Speedups]{Bar chart for the source lines of code changed for the different Polybench kernels. Each kernel has a cluster of four bars, one for each variant that is not the baseline.}
\end{figure}


\subsection{Experiment 2: Exhaustive Search for 2MM}
...
For the 2MM benchmark, we evaluated the performance of all possible format choices and compare their execution times against their model scores (the value of the objective function for that choice).
...
\begin{figure}
	\includegraphics[width=\columnwidth]{ScoreValidity/2mm-all.pdf}
	\caption{Execution time and model score of all possible format choices for the 2MM kernel. The model selects the choice with the lowest score. Outlier shown in red. Y axis is log-scale.}
	\label{2MMAllChoices}
...
Types of approaches / topics:
\begin{enumerate}
	\item schedule based transformations
		\item tiling
	\item compiler only approaches / no user input: \cite{cierniak1995unifying}
	\item no model approaches
	\item 
\end{enumerate}
...
\cite{henretty2011data} uses specialized data layout transformations to optimize stencil computations for SIMD architectures.

\cite{kronawitter2018automatic} does automatic and user-exposed layout transformations for stencil codes. arbitrary affine transformations built into ExaSlang (DSL).

\cite{chen2005constraint,chen2005integrating,ozturk2011data} use a constraint network approach. No transformations between computations, no user interface.

\cite{kandemir1998improving} combines schedule and data transformations. No user interface. Does consider multiple loops, but not format conversions.

\cite{zinenko2018modeling} only considers schedule transformations

\cite{cierniak1995unifying} combines schedule and data transformations. Only considers single loop nests. No user interface.

\cite{lu2012empirical} uses empirical cost components to select data layout transformations for tensor computations. tensor computation specialized. built into dsl

\cite{unat2016tida} presents abstractions for specifying loop tiling. does not do data transformations.
...
Finally, the data layout transformations presented in this paper work only in isolation. 
A critical element of future work will support the simultaneous specification of data layout and loop schedule transformations. 

\todo{cite isl somewhere}
\todo{cite rajaperf somewhere}
\todo{put the overall algorithm for the model selection}
