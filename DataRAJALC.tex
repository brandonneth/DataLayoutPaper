...
\begin{lstlisting}[caption={Main loopchain of the determinant update kernel}]
//tempMat = U^T * Ainv
for(i = 0; i < dc; i++) {
  for(j = 0; j < norb; j++) {
		tempMat(i,j) = 0;
    for(k = 0; k < norb; k++) {
			tempMat(i,j) += U(k,i) * Ainv(k,j);
		}
	}
}
...
\verb.tempMat. is indexed by the expression \verb.delay_list(i).. 
...
Our first \FormatDecisions{} variant seeks to identify a performance baseline to get a sense of teh overhead of using \FormatDecisions{}:
\begin{lstlisting}
...
\begin{lstlisting}
	auto dec = format_decisions(tie(U_, Ainv_, tempMat_, V_), 
	   knl1, knl2, knl3, knl4);
...
While an ever-so-slight reduction in execution time, not nearly enough to account for the overhead.
...
The next version creates the same loop chain object, but does not use it:
\begin{lstlisting}
...
knl4();
\end{lstlisting}
It executes the same way as the RAJA baseline, but takes 23.44 seconds.
