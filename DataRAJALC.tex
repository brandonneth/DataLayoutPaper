...
\todo{Everywhere there is code, Michelle is crying for some comments.  See commented out
details for this one in latex}
%FIXME: Brandon, I would really like to see some comments in the below.  Specifically,
% // Array declarations with layout specifications
% Then put the POLICY decl first and then each loop body with each knl and a comment with the
% math it is doing.
% I didn't change this because I realize you will have to change line number references in the text
% so want you to decide.
\begin{figure}
	\begin{lstlisting}[
...
This section reviews how a computation is described in RAJA, working with the implementation of the 3MM kernel (i.e., three matrix multiplications) found in Listing~\ref{3MMStart}.
...
%expanding the declarative data optimization system begun by RAJALC.
...
Two come from the polybench~\cite{pouchet2012polybench} section of the RAJAPerf benchmark suite~\cite{hornung2017raja} and one based on the determinant update from the miniQMC miniapp~\cite{richards2018fy18}.
...
\todo{I still need to run this again bc the code has changed and I'm not lookig at the RAJA\_OpenMP variant anymore.}
