...
\item Format-Conversion Matching: At each conversion point, the input format for the conversion matches the format of the immediately preceding kernel. Similarly, the output format for the conversion matches the format of the immediately succeeding kernel.
...
Symbolically, we can construct the model's constraints as follows.
To start, let $F$, $K$, and $T$ represent the set of all formats, kernels, and conversion times.
Because there is one more conversion than kernel, we know that $|T| = |K| + 1$.
Furthermore, we add an extra elements to $K$ to represent the formats before any computation occurs and after all the computations are complete.
Next, we can enumerate our decision variables.
For the format decision variables, we have one per format per computation. 
Let $fmt_{f,k}$ denote the variable for using the format $f$ during kernel $k$.
Similarly for the conversion decision variables, let $conv_{i,o,t}$ denote the decision variable for converting from format $i$ to to format $o$ at conversion time $t$.
With our variables defined, we can add our constraints.
The Format Uniqueness constraints are given by:
\begin{align*}
	\bigwedge\limits_{k \in K} (1 = \sum\limits_{f \in F} fmt_{f,k})
\end{align*}
Conversion Uniqueness constraints are similar:
\begin{align*}
	\bigwedge\limits_{t \in T} (1 = \sum\limits_{i \in F}\sum\limits_{o \in F} conv_{i,o,t})
\end{align*}
Format Conversion Matching constraints have two parts. 
The first matches the inputs of the conversions to the format at the preceding kernel:
\begin{align*}
	\bigwedge\limits_{t \in T} \bigwedge\limits_{i \in F} fmt_{i,t.prev} = \sum\limits_{o \in F} conv_{i,o,t}
\end{align*}
In this formulation, $t.prev$ represents the kernel before $t$. 
Outputs are matched similarly:
\begin{align*}
	\bigwedge\limits_{t \in T} \bigwedge\limits_{o \in F} fmt_{o,t.next} = \sum\limits_{i \in F} conv_{i,o,t}
\end{align*}
Finally, if we represent all the user choices $U$ as pairs of kernels and the formats the user wants, we can represent the constraints as:
\begin{align*}
