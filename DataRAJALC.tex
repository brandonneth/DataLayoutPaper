...
Particularly challenging is achieving good memory performance in a codebase run on multiple heterogeneous systems.
When performance considerations --- architecture, memory hierarchy configuration, even input data characteristics --- are hard-coded into the application's code, improved memory performance comes at the cost of maintainability and portability. 
...
%NEed to pivot to portabilitiy libraries
...
We focus here on data layout transformations between canonical\footnote{By canonical, we mean row-major and column-major layouts, as well as their higher dimensional analogues.} data layouts.
...
\end{itemize}
We evaluate our contributions by comparing its performance and source lines of code (SLOC) changed/added to hand-implemented transformations.
...
While these lambdas can optionally accept additional parameters for other values, this capability is not used here.
...
The first option is the only one that still uses RAJA and maintains the abstraction of a two dimensional loop: write a kernel that has a rectangular iteration space and guards iterations that are not part of the triangle.
The second option sticks with RAJA but loses the abstraction of a two dimensional loop entirely: construct and traverse an index array.
The third option sets aside RAJA entirely to maintain the abstraction of a two dimensional loop: use regular C++.
...
% \begin{lstlisting}
% // Reference Triangular Loop Nest
% for(int i = 0; i < N; i++) {
% 	for(int j = i+1; j < M; j++) {
% 		bar(i,j);
% 	}
...
% //Option 1: Guard
% auto iSeg = RangeSegment(0,N);
% auto jSeg = RangeSegment(0,M);
% auto iterationSpace = make_tuple(iSeg,jSeg);
...
% 	}
% };
% kernel<2DPOLICY>(iterationSpace, lambda1);
...
% for(int i = 0; i < N; i++) {
% 	size_t numJs = M - (i+1);
% 	std::vector<int> iTemp(numJs, i);
% 	iVals.insert(iVals.end(), iTemp.begin(), iTemp.end());
% 	std::vector<int> jTemp(numJs);
...
Most importantly, this means that the kernels cannot make indirect accesses (\verb.a(b(i)).), or call functions within the lambdas that use the loop iterators.

Regardless of whether the user has used the model or not, 
after the user has registered their choices, the complete computation with interspersed format conversions is generated using the \verb.generate(). method.
...
The next three subsections describe how we set up the decision space and constraints, how we construct the objective function, and how we estimate the cost coefficients for the variables in the model.
...
Two types of costs need estimation: use costs, which represent the estimated cost of using a particular layout for a kernel, and conversion costs, which represent the cost of converting from one format to another.
...
This fully normalized access order is then used as the layout order in the generic modeling function.

\todo{need to connect from this result to the kernel that actually does the estimation}
...
%%%%%%%%%%%%%%%%%%%%%%
\section{Experiment 1: RAJAPerf Benchmark Suite}
...
\section{Case Study: Quantum Monte Carlo Simulation}
...
The results of our evaluation are mixed, identifying layout transformations that improve performance compared to not making transformations. 
However, while performance improves compared to a version that uses \FormatDecisions{} without any transformations, it is overall slower than a version that immediately executes the kernel objects rather than grouping them in a container.
...
\begin{lstlisting}[caption={Main loopchain of the determinant update kernel. Note that the access/storage order here is reversed.}]
//tempMat = U^T * Ainv
for(i = 0; i < dc; i++) {
  for(j = 0; j < norb; j++) {
		tempMat(i,j) = 0;
...
However, this limitation should not restrict the programmer's ability to explore layout changes without the help of the performance model. 
To overcome this limitation, \FormatDecisions{} must only generate symbolic evaluation code when instructed. 
This led us to incorporate the \verb.model(). template method. 
All symbolic evaluation is performed here, so code that does not make a call to the method need not limit their kernels to the accesses supported by RAJALC symbolic evaluation.


The RAJA reference implementation makes calls to each of the kernels in sequence:
\begin{lstlisting}
knl1();
knl2();
knl3();
knl4();
\end{lstlisting}
Its execution time is 10.18 seconds. All times reported here are for the 1 2 2 shape.

Our first \FormatDecisions{} variant seeks to identify a performance baseline to get a sense of the overhead of using \FormatDecisions{}:
\begin{lstlisting}
auto dec = format_decisions(tie(U, Ainv, tempMat, V), 
  knl1, knl2, knl3, knl4);
auto comp = dec.generate();
comp();
\end{lstlisting}
Its execution time is 23.63 seconds. A significant slowdown that we will return to exploring momentarily, after identifying any layout transformations that reduce it.


The exploration of possible data layout transformations begins with identifying views that are accessed out of order. 
Four matrices appear hear: \verb.Ainv., \verb.U., \verb.V., and \verb.tempMat..
We start with all layout changes with mismatch orderings between the schedule and loopchain. Those choices are here:
\begin{lstlisting}
	auto dec = format_decisions(tie(U, Ainv, tempMat, V), 
	   knl1, knl2, knl3, knl4);
	...
	comp();		
\end{lstlisting}
This fully transformed version executes in 27.28 seconds, slower than the \FormatDecisions{} baseline.

To refine the transformation sequence, we note that the access to Ainv in the final kernel does not change most iterations, as it does not use the innermost loop index. 
Our second transformation sequence uses this fact, making format choices only on views that mismatch and use the innermost index. Those choices are here:
\begin{lstlisting}
	auto dec = format_decisions(tie(U, V), knl1, knl2, knl3, knl4);
	dec.set_format_for(U, {0,1}, knl1);
	dec.set_format_for(U, {1,0}, knl4);
	dec.set_format_for(V, {1,0}, knl3);
	auto comp = dec.generate();
	comp();
\end{lstlisting}
...
The third and fourth versions explore which format changes caused the performance improvement.
Version three isolates \verb.U.:
\begin{lstlisting}
	auto dec = format_decisions(tie(U), knl1, knl2, knl3, knl4);
	dec.set_format_for(U, {0,1}, knl1);
	dec.set_format_for(U, {1,0}, knl4);
	auto comp = dec.generate();
	comp();
\end{lstlisting}
It executes in 22.30 seconds.
Version four isolates \verb.V..
\begin{lstlisting}
	auto dec = format_decisions(tie(V), knl1, knl2, knl3, knl4);
	dec.set_format_for(V, {1,0}, knl3);
	auto comp = dec.generate();
	comp();
\end{lstlisting}
It executes in 23.43 seconds, indicating that most of the performance improvements come from the transformations applied to \verb.U..

With a useful layout optimization isolated, we return to the significant difference between the original RAJA baseline code and the \FormatDecisions{} baseline.
...
To explore this cause, we return to the baseline implementation and wrap these components in an additional timer:
\begin{lstlisting}
firstGuessTimer->start();
auto dec = format_decisions(tie(U, Ainv, tempMat, V), 
  knl1, knl2, knl3, knl4);
auto comp = dec.generate();
firstGuessTimer->start();
comp();
\end{lstlisting}
This timer clocks a vanishly small contribution: 0.0022 seconds.
...
\begin{lstlisting}
auto chain = loop_chain(knl1, knl2, knl3, knl4);
chain();
\end{lstlisting}
...
We can confirm this with a version that creates the loop chain object, but does not use it:
\begin{lstlisting}
