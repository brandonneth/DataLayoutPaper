...
For \textsc{2mm}, we see a 11.1\% speedup for the \verb.Hand_Layout. variant and 11.0\% speedups for \verb.Format_Decision. and \verb.Model_Choice. 
...
The \verb.Format_Decision. variant does slightly better with speedups of 5.2\%, 8.6\%, and 9.6\%.
...
\textsc{qmc} fares similarly to \textsc{doitgen}, although it sees more significant slowdowns. 
At the standard problem size, \verb.Hand_Layout. and \verb.Format_Decision. see small slowdowns of 0.5\% and 1.8\%. 
The \verb.Model_Choice. has a more significant slowdown at 10.9\% due to the overhead costs of running the model.
For the larger problem sizes, this cost has less influence, at 2.9\% at SizeFactor=2 and 0.5\% at SizeFactor=4. 
Unlike the other kernels, the transformed data in \textsc{qmc} are not square matrices. Because the matrices have a constant width of 64, the possible performance improvements are smaller.
This is because even in an nonoptimal data layout, consecutively accessed elements of the matrix may still be loaded into cache together. 
Furthermore, the absolute distance between consecutively accessed elements in memory remains constant as the problem size increases, so larger problem sizes do not change the available performance improvement.
Combining the smaller available performance improvement with the cost of layout transformations led to an overall performance reduction.
To confirm this cause, we increased the width of the matrices from 64 to 128 and 192 and and for SizeFactor=4 found speedups of 2.2\% and 3.9\%, respectively. 
...
The standard problem size gives a 1.8\% speedup for \verb.Hand_Layout., a more significant 13.5\% for \verb.Format_Decision., and a slowdown of 13.3\% for \verb.Model_Choice.. 
...
Overall, data format is an important consideration when optimizing applications.
...
\FormatDecisions{} remedies this situation by combining declarative format specification  interface with runtime performance modeling to supplement user directives.
Integrated directly into RAJA, but possible in any library with computation objects and a View-like abstraction, \FormatDecisions{} provides a flexible interface for quick exploration of data layout changes. 
